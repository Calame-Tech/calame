...
import type { EmbeddingClient, RagDocument, RagFolder, RagSource, VectorStore } from '../types.js';
...
export class IngestionPipeline {
  ...
  async ingestDocument(input: IngestDocumentInput): Promise<RagDocument> {
    ...
    const persist = this.db.transaction(() => {
      if (existing) {
        ...
        this.db.prepare(`DELETE FROM rag_chunks WHERE document_id = ?`).run(existing.id);
        this.db
          .prepare(
          ...
          .run(folderId, name, input.mimeType, size, hash, input.etag ?? null, now, existing.id);
      } else {
        this.db
          .prepare(
    ...
    const refreshed = this.db
      ...
      .get(documentId);
    if (!refreshed)
      throw new Error(`IngestionPipeline: document ${documentId} vanished after insert`);
  ...
  markDocumentDeleted(documentId: string): void {
    ...
    const apply = this.db.transaction(() => {
      ...
      this.db.prepare(`UPDATE rag_documents SET deleted_at = ? WHERE id = ?`).run(now, documentId);
    });
    apply();
  }
}
