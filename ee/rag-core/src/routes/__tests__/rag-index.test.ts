...
function readJob(db: BetterSqlite3Database, sourceId: string): RagJob {
  const row = db
    .prepare(`SELECT * FROM rag_jobs WHERE source_id = ? ORDER BY started_at DESC LIMIT 1`)
...
describe('registerRagIndexRoutes — background sync (HTTP 202 + queue)', () => {
  ...
  it('two POSTs back-to-back on the same source: the second returns 409 Conflict', async () => {
    ...
    const count = db
      .prepare<[string], { c: number }>(`SELECT COUNT(*) AS c FROM rag_jobs WHERE source_id = ?`)
      .get(source.id);
...
describe('runSyncJob — monthly cap kill-switch', () => {
  ...
  it('marks the job failed and emits `rag.sync.failed` with reason=cap_exceeded', async () => {
    ...
    expect(
      (pipeline as unknown as { markDocumentDeleted: ReturnType<typeof vi.fn> })
        .markDocumentDeleted,
