...
describe('registerRagSourcesRoutes — polling integration', () => {
  ...
  it('GET source includes pollingIntervalSeconds in the response', async () => {
    ...
    await captured.get('/api/rag/sources/:id')(makeReq({ params: { id: created.id } }), getRes.res);
...
describe('schema migration — v6 tenant_id', () => {
  ...
  it('upgrade path — v5 DB without the column gets it added by v6, existing rows default to "default"', () => {
    ...
    const legacy = db.prepare(`SELECT tenant_id FROM rag_sources WHERE id = ?`).get('s-legacy') as {
      tenant_id: string;
    };
...
describe('registerRagSourcesRoutes — soft delete', () => {
  ...
  it('DELETE marks deleted_at and keeps the row in DB (soft delete)', async () => {
    ...
    const delRes = makeRes();
    await captured.delete('/api/rag/sources/:id')(makeReq({ params: { id } }), delRes.res);

    expect(delRes.statusCode).toBe(200);
    // Row still exists.
    const row = db.prepare(`SELECT id, deleted_at FROM rag_sources WHERE id = ?`).get(id) as {
      id: string;
  ...
  it('DELETE a soft-deleted source returns 410 Gone', async () => {
    ...
    await captured.delete('/api/rag/sources/:id')(makeReq({ params: { id } }), firstDel.res);
    expect(firstDel.statusCode).toBe(200);

    const secondDel = makeRes();
    await captured.delete('/api/rag/sources/:id')(makeReq({ params: { id } }), secondDel.res);
...
describe('registerRagSourcesRoutes — permanent delete', () => {
  ...
  it('DELETE /:id/permanent cascades chunks, docs, folders, jobs and calls vectorStore.deleteByDocument', async () => {
    ...
    // Everything's gone.
    expect(db.prepare(`SELECT COUNT(*) AS c FROM rag_sources WHERE id = ?`).get(id)).toEqual({
      c: 0,
    });
    expect(db.prepare(`SELECT COUNT(*) AS c FROM rag_folders WHERE source_id = ?`).get(id)).toEqual(
      { c: 0 },
    );
    expect(
      db.prepare(`SELECT COUNT(*) AS c FROM rag_documents WHERE source_id = ?`).get(id),
    ).toEqual({ c: 0 });
    expect(db.prepare(`SELECT COUNT(*) AS c FROM rag_chunks`).get()).toEqual({ c: 0 });
    expect(db.prepare(`SELECT COUNT(*) AS c FROM rag_jobs WHERE source_id = ?`).get(id)).toEqual({
      c: 0,
    });
...
describe('registerRagSourcesRoutes — Phase B tenant isolation', () => {
  ...
  it('DELETE /api/rag/sources/:id across tenants returns 404 and keeps the row active', async () => {
    ...
    // Row is still active.
    const row = db.prepare(`SELECT deleted_at FROM rag_sources WHERE id = ?`).get(id) as {
      deleted_at: string | null;
    };
