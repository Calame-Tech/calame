  ...
  if (current < 6) {
    ...
    for (const table of ragTables) {
      if (hasTable(raw, table)) {
        addColumnIfMissing(raw, table, 'tenant_id', `TEXT NOT NULL DEFAULT '${DEFAULT_TENANT_ID}'`);
        raw.exec(`CREATE INDEX IF NOT EXISTS idx_${table}_tenant ON ${table}(tenant_id)`);
      }
    }
