...
export default function FolderTreeView({ source, refreshKey = 0 }: FolderTreeViewProps) {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      setChildren((prev) => ({
        [ROOT_KEY]: prev[ROOT_KEY] ?? { folders: [], documents: [], loading: false, error: null },
      }));
      setExpanded(new Set());
    } catch (err) {
      const message =
    ...
    return (
      <li key={folder.id} className="select-none">
        ...
        >
          ...
          <span className="text-xs text-gray-600 font-mono-plex truncate ml-1">{folder.path}</span>
        </button>
        {isOpen && (
          <>
            ...
            {folderChildren && !folderChildren.loading && !folderChildren.error && (
              <ul className="space-y-0.5">
                ...
                {folderChildren.folders.length === 0 && folderChildren.documents.length === 0 && (
                  <li
  ...
  const renderDocument = (doc: RagDocument, depth: number): ReactNode => {
    ...
    return (
      ...
      >
        <FileIcon />
        <span className={`text-sm truncate ${hasIngestError ? 'text-gray-400' : 'text-gray-300'}`}>
          {doc.name}
        </span>
        {hasIngestError && (
          <span
