...
/**
 * In-process port of scripts/generate-demo-db.js (kept at the repo root for the standalone
 * `node scripts/generate-demo-db.js` monorepo dev workflow — that file is the canonical
 * source, this is a faithful mechanical port of its schema and seed logic).
 *
 * The route handler (POST /api/connections/demo) used to spawn that script as a child
 * process, which relies on `process.cwd()/scripts/...` and `process.execPath` — neither is
 * valid once Calame runs as a packaged, bundled single file with no monorepo checkout around
 * it. This module generates the exact same "logistique" demo database directly, synchronously,
 * with no child process and no filesystem lookup for a script path.
 *
 * Tables: client, livreur, depot, zone, colis, tournee, tournee_colis, incident, paiement,
 * notification, historique_statut. Volume: ~40 000 rows total.
 */
...
const CHAINS_PAR_STATUT: Record<string, Array<[string | null, string]>> = {
...
/**
 * Generate the demo "logistique" SQLite database at `dbPath`, overwriting it if it already
 * exists. Synchronous (better-sqlite3 is synchronous end to end), matches the schema and seed
 * data produced by scripts/generate-demo-db.js.
 */
export function generateDemoDb(dbPath: string): void {
  ...
  const insertMany = db.transaction(
    (stmt: Database.Statement, rows: Array<Record<string, unknown>>) => {
  ...
  const zones: number[] = (db.prepare('SELECT id FROM zone').all() as IdRow[]).map((r) => r.id);
  ...
  for (let i = 0; i < 20; i++) {
    ...
    depots.push(Number(r.lastInsertRowid));
  }
  ...
  for (let i = 0; i < 500; i++) {
    ...
    clients.push(Number(r.lastInsertRowid));
  }
  ...
  for (let i = 0; i < 50; i++) {
    ...
    livreurs.push(Number(r.lastInsertRowid));
  }
...
VALUES (@reference, @id_client, @id_livreur, @id_depot, @poids_kg, @longueur_cm, @largeur_cm, @hauteur_cm, @statut, @priorite, @fragile, @valeur_declaree, @adresse_livraison, @ville_livraison, @code_postal_livraison, @date_creation, @date_expedition, @date_livraison_prevue, @date_livraison_reelle, @tentatives_livraison, @commentaire, @signature_requise)`);
  ...
  const colisIds: number[] = [];
  // colisId → { dateCreation, statut } pour garantir la cohérence des tables dépendantes
  const colisMetaMap = new Map<number, { dateCreation: string; statut: string }>();
  ...
  for (let i = 0; i < TOTAL_COLIS; i++) {
    ...
    const colisId = Number(r.lastInsertRowid);
    colisIds.push(colisId);
    colisMetaMap.set(colisId, { dateCreation, statut });
  }
  ...
  const colisInTournee = new Set<number>();
  ...
  for (let i = 0; i < 300; i++) {
    ...
    const tourneeId = Number(r.lastInsertRowid);
  ...
  function statutPaiementPourColis(colisStatut: string): string {
