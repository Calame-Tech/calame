...
export default function ChatPanel({ selectedTables, activeProfiles }: ChatPanelProps) {
  ...
  const [profileMeta, setProfileMeta] = useState<
    Record<string, { aiSettings?: Array<{ name: string; label: string }> }>
  >({});
  const [selectedProfile, setSelectedProfile] = useState<string | null>(activeProfiles[0] ?? null);
  ...
  return (
    <div>
      ...
      {/* Chat area */}
      <div className="card-primary flex flex-col" style={{ height: '400px' }}>
        {/* Messages */}
