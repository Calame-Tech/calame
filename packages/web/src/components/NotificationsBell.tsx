...
export default function NotificationsBell({ onNavigate }: NotificationsBellProps) {
  const t = useTranslations('setupLogin.notificationsBell');
  ...
  const formatRelativeTime = (iso: string): string => {
    ...
    if (diffSec < 60) return tCommon('justNow');
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      >
        ...
        {unreadCount > 0 && (
          <span
            aria-label={t('ariaUnreadCount', { count: unreadCount })}
        ...
        createPortal(
          ...
          >
            <div className="flex items-center justify-between px-3 py-2 border-b border-gray-800">
              <span className="text-sm font-medium text-gray-200">{t('panel.title')}</span>
              <button
                type="button"
            ...
            {notifications.length === 0 ? (
              <div className="p-4 text-sm text-gray-500 text-center">{t('panel.empty')}</div>
            ) : (
              <ul>
                {notifications.map((n) => (
